...
describes what users get. Pick a format with `-o`, or let rickandmorty choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
rickandmorty <command> -o table   # aligned columns for reading
rickandmorty <command> -o jsonl   # one JSON object per line, for piping
rickandmorty <command> -o json    # a single JSON array
rickandmorty <command> -o csv     # spreadsheet friendly
rickandmorty <command> -o tsv     # tab-separated
rickandmorty <command> -o url     # just the URL column
rickandmorty <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
rickandmorty <command> --fields id,title,url
```
...
```bash
rickandmorty <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
rickandmorty <command>            # a table, because this is a terminal
rickandmorty <command> | wc -l    # JSONL, because this is a pipe
```
